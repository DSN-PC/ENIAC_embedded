 ...
 * Vas Ádám             11/08/13 Added a line to store the server's IP address and port
 ********************************************************************/
#define __ANNOUNCE_C
...
extern IP_ADDR clientAddress;
extern WORD clientPort;
...
void DiscoveryTask(void)
{
	...
	switch(DiscoverySM)
	{
            ...
            case DISCOVERY_LISTEN:
                ...
                // Change the server address and port of the ENIAC TCP clients
                // to the address of the sender of the discovery message
                if (mainServerAddress.Val != remoteNode.IPAddr.Val)
                    mainServerAddress = remoteNode.IPAddr;
                if (mainServerPort != UDPSocketInfo[MySocket].remotePort)
                    mainServerPort = UDPSocketInfo[MySocket].remotePort;
